...
and we are going to try to recreate it in Python. We will use the PyGame
module, which (unfortunately) only works on Python version 2 on the Raspberry
Pi so open \emph{Idle} instead of \emph{Idle3.} The first bit of code to type in
just creates the game window and runs the game loop.

\pythonscript{step1}{The main game loop}
